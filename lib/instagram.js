...
async function getProfileInfo(page) {
  // Don't navigate — detect username from current page state
  const url = page.url();
  ...
  const data = await apiFetch(page, `https://www.instagram.com/api/v1/users/web_profile_info/?username=${username}`).catch(() => null);
